...
This method requires an upstream external builder image, from Red Hat. It also requires that your source code is in a "git style" repository.

In this example, I'm using the following example code:

[source,bash]
$ oc project user30-python-containers

image:add-s2i-openshift.png[title="Add a Git Project"]
