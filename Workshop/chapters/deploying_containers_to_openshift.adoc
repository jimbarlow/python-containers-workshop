...
That build method is supposed to automatically detect the application, and launch it with the Gunicorn WSGI HTTP server. How can we be sure, in case we happen to be skeptical, or 
just curious?
...
```
We see that indeed, there are multiple threads of Gunicorn running rout Flask application.
